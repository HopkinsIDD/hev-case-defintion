...
for (batch in 1:num_batches) {
  ...
  for (i in start_i:end_i) {
    ...
    batch_case_defs[[case_def_name]] <- apply(dat, 1, function(row) {
      any(row[case_def] == 1)
    })
  }
  ...
  saveRDS(batch_df, file = paste0("case_def_batch_joined", batch, ".rds"))
}
...
for (batch in 1:num_batches) {
  path <- here("scripts", "Case_definitions", paste0("case_def_batch_joined", batch, ".rds"))
...
saveRDS(results_df, file = "joint_final_results_or_combos.rds")
...
case_definitions[[35]]
tabyl(dat, case_def35, infect_bin)
...
# Get weighted counts
svy_table <- svytable(~ case_def35 + infect_bin, design = svy_design)
...
# Try filtering to top 10 or top 20?
top <- metrics_spec_top[1:10, ]

top_sens <- top %>% arrange(desc(sensitivity))
  
# Existing case defs?
# manual_points <- data.frame(
#   sensitivity = c(0.60, 0.70),
#   specificity = c(0.55, 0.52),
#   label = c("MSF", "WHO", "IMC")  # Optional labels
# )

plot <- ggplot(top, aes(x = specificity, y = sensitivity)) +
  geom_point(size = 3, color = "steelblue") +
  # geom_errorbar(aes(ymin = sensitivity_lower, ymax = sensitivity_upper), width = 0.005) +
  # geom_errorbarh(aes(xmin = specificity_lower, xmax = specificity_upper), height = 0.005) +
  labs(
    x = "Specificity",
    y = "Sensitivity") +
  theme_minimal() +
  ...
  ) + xlim(0.5, 0.7) + ylim(0.5, 0.7)

# With shading
plot <- ggplot(top, aes(x = specificity, y = sensitivity)) +
  # Add shaded region for specificity < 0.6
  geom_rect(aes(xmin = 0.5, xmax = 0.6, ymin = 0.5, ymax = 0.6),
            fill = "mistyrose", alpha = 0.4, inherit.aes = FALSE) +
  # Add shaded region for specificity >= 0.6
  geom_rect(aes(xmin = 0.6, xmax = 0.7, ymin = 0.5, ymax = 0.6),
            fill = "aliceblue", alpha = 0.4, inherit.aes = FALSE) +
